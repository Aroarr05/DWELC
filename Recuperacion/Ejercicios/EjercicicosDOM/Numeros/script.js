...
let contadorClicks = new Map();

celdas.forEach(celda => {
    contadorClicks.set(celda, 0);

    celda.addEventListener('mouseenter', entrando);
    celda.addEventListener('mouseleave', saliendo);

    celda.addEventListener('click', () => {
        let clicks = contadorClicks.get(celda) + 1;
        contadorClicks.set(celda, clicks);
        celda.textContent = clicks;
    });
});